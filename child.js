...
class GameMapChild {
    ...
    handleTicktack(res) {
        setImmediate(() => {
            try {
                ...
                console.log("[GameMapChild] Error in parseTicktack:");
            } catch (error) {
                console.error("[GameMapChild] Error in parseTicktack:", error);
            }
        });
    }
    ...
    async checkingGameStatus(res) {
        try {
            ...
            const enemies = res.map_info.players.filter(p => p.id !== this.playerId && p.id !== this.playerIdParent);
            ...
            if(res.tag == "player:stun-by-weapon" && res.player_id != this.playerId && res.player_id != this.playerIdChill) {
                ...
                if(currentPlayer.currentWeapon !== 2) {
                    await this.socket.emit('action', { action: "switch weapon", "characterType": "child" });
                }
            ...
            if(pathToEnemy) {
                ...
                if(pathToEnemy.length > 1) {
                    if(currentPlayer.currentWeapon !== 2) {
                        this.socket.emit('action', { action: "switch weapon", "characterType": "child" });
                    }
                ...
                if(currentPlayer.currentWeapon == 2) {
                    ...
                    await this.socket.emit(SOCKET_EVENTS.DRIVE_PLAYER, { direction: pathToEnemy, "characterType": "child" });
            ...
            if(boxPath) {
                ...
                if (this.hasValueBALK(currentPlayer.currentPosition.row, currentPlayer.currentPosition.col)) {
                    this.isMoving = true;
                    return { type: EVENT_GAME.BOMBED, path: "b", tick: "BOMBED 1" };
                }
            }
    ...
    async emitDriver(event, data) {
        console.log({ direction: data, "characterType": "child"})
        await this.socket.emit(event, { direction: data, "characterType": "child"});
    }
